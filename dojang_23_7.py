...
for i in range(len(matrix)):        # 세로열 만큼 반복
  for j in range(len(matrix[i])):   # 가로열 만큼 반복 | 인덱싱으로 주변 요소를 찾아야 하기 때문에 range 사용
    if matrix[i][j] == '.':         # '*'은 따로 수정을 할 필요가 없으므로 '.'이 아니면 건너뜀
      ...
      # temp = []
      # for k in range(i-1, i+2):   -- 세로로 한 칸 주변에 있는지
      #   if k >= 0 and k < row:    -- 혹시 세로열 인덱스가 범위를 벗어나진 않았는지
      #     temp.append(matrix[k])  -- temp에 저장
      ...
      # for v in temp:                -- temp의 세로열 만큼 반복
      #   for k in range(j-1, j+2):   -- 가로로 한 칸 주변에 있는지
      #     if k >= 0 and k < col:    -- 혹시 가로열 인덱스가 범위를 벗어나진 않았는지
      #       temp.append(v[k])       -- temp 리스트에 이미 다른 요소가 있어서 실제 코드로 사용하려면 새로운 빈 리스트에 할당해야함

      # 각 요소마다 상하좌우로 1칸 근처에 있는 요소들을 저장한 temp라는 리스트가 만들어짐

      matrix[i][j] = str(temp.count('*')) # temp에서 별의 총 갯수를 세고(temp.count('*') 리스트를 간단하게 합칠 수 있게 join을 쓰기 전 class를 str으로 변환 후 '.'였던 요소에 대입
      
  print(''.join(matrix[i]))   # matrix 리스트를 세로줄 별로(들여쓰기를 가로열 for문 종료에 맞춰줌) join 후 출력
